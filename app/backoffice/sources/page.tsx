...
export default function SourcesPage() {
  ...
  const createMut = useMutation({
    mutationFn: () => createSource(token!, { name: newName, label: newLabel }),
    onSuccess: () => {
  ...
  const columns: ColumnDef<BibliographicSource>[] = [
    {
      ...
      cell: ({ row }) => (
        <InlineEdit
          ...
          onSave={(name) => updateMut.mutate({ id: row.original.id, data: { name } })}
        />
      ),
    },
    {
      ...
      cell: ({ row }) => (
        <InlineEdit
          ...
          onSave={(label) => updateMut.mutate({ id: row.original.id, data: { label } })}
        />
      ),
      size: 150,
    },
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Bibliographic Sources</h1>
          <p className="text-sm text-muted-foreground">Manage bibliographic source records</p>
        </div>
      </div>
