              ...
              <div className='space-y-3'>
                ...
                >
                  ...
                  <div className='flex-1 min-w-0'>
                    ...
                    <p className='text-sm text-muted-foreground mt-1'>
                      Define the structural parts of a letter (e.g., ascender,
                      body, descender)
                    </p>
                    <p className='text-xs text-muted-foreground mt-2'>
                      {(components.data ?? []).length} component{(components.data ?? []).length !== 1 ? 's' : ''} so far
                    </p>
                  </div>
                </button>
                <button
                ...
                >
                  ...
                  <div className='flex-1 min-w-0'>
                    ...
                    <p className='text-sm text-muted-foreground mt-1'>
                      Define the visual characteristics (e.g., straight, curved,
                      split)
                    </p>
                    <p className='text-xs text-muted-foreground mt-2'>
                      {(features.data ?? []).length} feature{(features.data ?? []).length !== 1 ? 's' : ''} so far
                    </p>
                  </div>
                </button>
                <button
                ...
                >
                  ...
                  <div className='flex-1 min-w-0'>
                    ...
                    <p className='text-sm text-muted-foreground mt-1'>
                      Add characters and assign allographs with their
                      component-feature combinations
                    </p>
                    <p className='text-xs text-muted-foreground mt-2'>
                      {(characters.data ?? []).length} character{(characters.data ?? []).length !== 1 ? 's' : ''} so far
                    </p>
                  </div>
                </button>
              </div>
          ...
          <div className='flex h-full items-center justify-center'>
            <div className='text-center space-y-4'>
              ...
              <p className='text-xs text-muted-foreground'>
                {(characters.data ?? []).length} characters · {(components.data ?? []).length} components · {(features.data ?? []).length} features
              </p>
            </div>
          </div>
        )}
