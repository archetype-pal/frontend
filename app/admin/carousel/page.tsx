...
function SortableCarouselItem({
  ...
  item: CarouselItemAdmin
  onUpdate: (id: number, data: Partial<CarouselItemAdmin>) => void
  onDelete: (item: CarouselItemAdmin) => void
}) {
  const {
    attributes,
    listeners,
...
export default function CarouselPage() {
  ...
  const { data: items } = useQuery({
    queryKey: adminKeys.carousel.all(),
    queryFn: () => getCarouselItems(token!),
    enabled: !!token,
  })

  const invalidate = useCallback(
    () => queryClient.invalidateQueries({ queryKey: adminKeys.carousel.all() }),
    [queryClient]
  )
  ...
  const handleDragEnd = useCallback(
    (event: DragEndEvent) => {
      ...
      queryClient.setQueryData(adminKeys.carousel.all(), updated)
  ...
  const handleUpdate = useCallback(
    (id: number, data: Partial<CarouselItemAdmin>) => {
