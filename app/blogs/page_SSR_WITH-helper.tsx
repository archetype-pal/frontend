// Clean Helper-based Refactor (with shared logic)
...
async function getBlogsList(offset: number, limit: number): Promise<{ results: Publication[], count: number }> {
  try {
...
export default async function BlogsList({ searchParams }: Props) {
  ...
  const { results: blogsList, count: total } = await getBlogsList(offset, limit)
