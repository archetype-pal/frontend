...
export function LightboxMeasurementTool({ containerRef, onClose }: LightboxMeasurementToolProps) {
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    ...
    if (!isMeasuring) {
      ...
      const distance = Math.hypot(
        end.x - currentMeasurement.start.x,
        end.y - currentMeasurement.start.y
      );
      ...
      {containerRect && (
        <div
          ...
          onMouseLeave={() =>
            ...
            setCurrentMeasurement((p) => ({ ...p, end: null }))
          }
        >
            ...
            {measurements.map((m) => (
              <g key={m.id}>
                <line
                  ...
                  strokeWidth="2"
                />
                <circle cx={m.start.x} cy={m.start.y} r="4" fill="rgba(59, 130, 246, 0.8)" />
                <circle cx={m.end.x} cy={m.end.y} r="4" fill="rgba(59, 130, 246, 0.8)" />
                <text
