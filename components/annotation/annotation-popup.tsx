...
export function AnnotationPopup({ annotation, onClose, onUpdate, style }: AnnotationPopupProps) {
  const [expandedSections, setExpandedSections] = React.useState(() => {
    if (annotation.selectedAllograph) {
      return Object.fromEntries(
        annotation.selectedAllograph.components.map((component) => [component.component_name, true])
      );
    }
    return {};
  });
  ...
  const updateComponent = (componentName: string, featureName: string, value: boolean) => {
    const newAnnotation = {
      ...annotation,
      components: {
      ...
      <div className="flex items-center justify-between border-b p-2">
        <div className="flex items-center gap-1">
          ...
          <div className="flex gap-1">
            <Button variant="ghost" size="icon" className="bg-green-500 hover:bg-green-600">
              <Check className="h-4 w-4 text-white" />
            </Button>
            <Button variant="ghost" size="icon" className="bg-red-500 hover:bg-red-600">
      ...
      <Tabs defaultValue="components">
                ...
                >
                  ...
                  <ChevronDown
                    className={`h-5 w-5 transform transition-transform ${
                      expandedSections[component.component_name] ? 'rotate-180' : ''
                    }`}
                  />
                </button>
                  ...
                  {component.features.map((feature) => (
                    <div key={feature.id} className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        checked={
                          annotation.components?.[component.component_name]?.[feature.name] ??
        ...
        <TabsContent value="notes" className="p-4">
          <textarea
            ...
            onChange={(e) => onUpdate({ ...annotation, content: e.target.value })}
