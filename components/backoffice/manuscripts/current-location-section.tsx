...
export function CurrentLocationSection({
  ...
  if (itemParts.length === 0) {
    return <SetupLocationPrompt historicalItemId={historicalItemId} />;
  }

  if (itemParts.length === 1) {
    return <SinglePartLocation historicalItemId={historicalItemId} part={itemParts[0]} />;
  }

  return <MultiPartSummary itemParts={itemParts} onNavigateToParts={onNavigateToParts} />;
}
...
function MultiPartSummary({
  ...
  const volumeCount = new Set(itemParts.map((p) => p.current_item).filter(Boolean)).size;
