...
export function backendToA9sAnnotation(backend: BackendGraph,
  imageHeight: number,
...
export function a9sToBackendFeature(a9s: Annotation): BackendFeature {
  const sel =
    (a9s.target as FragmentTarget)?.selector?.value ?? ''

  const m = sel.match(/xywh=pixel:(\d+(?:\.\d+)?),(\d+(?:\.\d+)?),(\d+(?:\.\d+)?),(\d+(?:\.\d+)?)/)
  if (!m) throw new Error('Annotation has no xywh FragmentSelector')
  const [, sx, sy, sw, sh] = m
  const x = parseFloat(sx), y = parseFloat(sy), w = parseFloat(sw), h = parseFloat(sh)
